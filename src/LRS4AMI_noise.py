...
def mix(idx):
    ...
    path = np.random.choice(list_noise)
    name_file = os.path.basename(path)
    id_noise = name_file.split(".")[0]
    ...
    T60 = np.random.uniform(hp.noise.RT[0], hp.noise.RT[1]) # Time for the RIR to reach 60dB of attenuation [s]
    ...
    # Save
    name_out = "{}_{}_{}.wav".format(idx,angle,id_noise)
    path_wav_out = os.path.join(dir_out,"noise",name_out)
